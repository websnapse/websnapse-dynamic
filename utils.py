...
def parse_rule(definition: str):
    ...
    """
    pattern = r"^((?P<bound>.*)\/)?(?P<consumption_bound>[a-z](\^((?P<consumed_single>[^0,1,\D])|({(?P<consumed_multiple>[2-9]|[1-9][0-9]+)})))?) \\to (?P<production>([a-z]((\^((?P<produced_single>[^0,1,\D])|({(?P<produced_multiple>[2-9]|[1-9][0-9]+]*)})))?;(?P<delay>[0-9]|[1-9][0-9]*))|(?P<forgot>0)))$"
